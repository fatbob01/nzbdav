...
)
{
    ...
    )
    {
        ...
        await CreateOrUpdateMirrorSymlink(mountFolder, error);
    ...
    private Task CreateOrUpdateMirrorSymlink(DavItem? mountFolder, string? error)
    {
        if (error != null || mountFolder is null) return Task.CompletedTask;

        var mirrorDir = configManager.GetSymlinkMirrorDir();
        if (mirrorDir is null) return Task.CompletedTask;

        try
        {
            var mountPath = NormalizePath(mountFolder.Path);
            var contentCategoryRoot = NormalizePath(Path.Join(DavItem.ContentFolder.Path, queueItem.Category));
            var comparison = GetPathComparison();
            var relativeContentPath = mountPath.StartsWith(contentCategoryRoot + Path.DirectorySeparatorChar, comparison)
                ? mountPath[(contentCategoryRoot.Length + 1)..]
                : mountFolder.Name;
            ...
            if (!string.IsNullOrWhiteSpace(mirrorParent))
            ...
            var targetIsDirectory = Directory.Exists(targetPath);
        ...
        {
            Log.Warning(e, $"Failed to create mirror symlink for `{queueItem.JobName}`.");
        }

        return Task.CompletedTask;
    }

    private static bool IsSymlinkUpToDate(string mirrorPath, string targetPath)
    {
        var desiredTarget = NormalizePath(targetPath);
        var linkTarget = GetSymlinkTarget(mirrorPath);
        if (linkTarget is null) return false;
        var comparison = GetPathComparison();
        return string.Equals(NormalizePath(linkTarget), desiredTarget, comparison);
    }

    private static string? GetSymlinkTarget(string mirrorPath)
    {
        var fileLinkInfo = new FileInfo(mirrorPath);
        if (fileLinkInfo.LinkTarget is not null)
            return ResolveLinkTargetPath(mirrorPath, fileLinkInfo.LinkTarget);

        var directoryLinkInfo = new DirectoryInfo(mirrorPath);
        if (directoryLinkInfo.LinkTarget is not null)
            return ResolveLinkTargetPath(mirrorPath, directoryLinkInfo.LinkTarget);

        return null;
    }

    private static string ResolveLinkTargetPath(string mirrorPath, string linkTarget)
    {
        var linkDirectory = Path.GetDirectoryName(mirrorPath);
        return linkDirectory is null
            ? linkTarget
            : Path.GetFullPath(linkTarget, linkDirectory);
    }

    private static void DeleteExistingPath(string mirrorPath)
    {
    ...
    private async Task RefreshMonitoredDownloads(ArrClient arrClient)
    {
        try
        {
            ...
            if (queueCount < 300) await arrClient.RefreshMonitoredDownloads();
        }
        catch (Exception e)
        {
